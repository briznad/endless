...
}});})();(function(){a.extend(a.effects,{version:"1.10.2",save:function(g,h){for(var f=0;f<h.length;f++){if(h[f]!==null){g.data(b+h[f],g[0].style[h[f]]);
...
a.easing["easeOut"+f]=function(g){return 1-e(1-g);};a.easing["easeInOut"+f]=function(g){return g<0.5?e(g*2)/2:1-e(g*-2+2)/2;};});})();})(jQuery);
/*!
 * jQuery.ScrollTo
...
};a.defaults={axis:"xy",duration:parseFloat(c.fn.jquery)>=1.3?0:1,limit:true};a.window=function(d){return c(window)._scrollable();};c.fn._scrollable=function(){return this.map(function(){var e=this,d=!e.nodeName||c.inArray(e.nodeName.toLowerCase(),["iframe","#document","html","body"])!=-1;
if(!d){return e;}var f=(e.contentWindow||e).document||e.ownerDocument||e;return/webkit/i.test(navigator.userAgent)||f.compatMode=="BackCompat"?f.body:f.documentElement;
...
e=e||d.duration;d.queue=d.queue&&d.axis.length>1;if(d.queue){e/=2;}d.offset=b(d.offset);d.over=b(d.over);return this._scrollable().each(function(){if(f==null){return;
}var l=this,j=c(l),k=f,i,g={},m=j.is("html,body");switch(typeof k){case"number":case"string":if(/^([+-]=)?\d+(\.\d+)?(px|%)?$/.test(k)){k=b(k);break;}k=c(k,this);
if(!k.length){return;}case"object":if(k.is||k.style){i=(k=c(k)).offset();}}c.each(d.axis.split(""),function(q,r){var s=r=="x"?"Left":"Top",u=s.toLowerCase(),p="scroll"+s,o=l[p],n=a.max(l,r);
if(i){g[p]=i[u]+(m?0:o-j.offset()[u]);if(d.margin){g[p]-=parseInt(k.css("margin"+s))||0;g[p]-=parseInt(k.css("border"+s+"Width"))||0;}g[p]+=d.offset[u]||0;
if(d.over[u]){g[p]+=k[r=="x"?"width":"height"]()*d.over[u];}}else{var t=k[u];g[p]=t.slice&&t.slice(-1)=="%"?parseFloat(t)/100*n:t;}if(d.limit&&/^\d+$/.test(g[p])){g[p]=g[p]<=0?0:Math.min(g[p],n);
}if(!q&&d.queue){if(o!=g[p]){h(d.onAfterFirst);}delete g[p];}});h(d.onAfter);function h(n){j.animate(g,e,d.easing,n&&function(){n.call(this,f,d);});}}).end();
};a.max=function(j,i){var h=i=="x"?"Width":"Height",e="scroll"+h;if(!c(j).is("html,body")){return j[e]-c(j)[h.toLowerCase()]();}var g="client"+h,f=j.ownerDocument.documentElement,d=j.ownerDocument.body;
return Math.max(f[e],d[e])-Math.min(f[g],d[g]);};function b(d){return typeof d=="object"?d:{top:d,left:d};}})(jQuery);(function(i){var g={},b,k,h,e=750,a;
function c(m){return"tagName" in m?m:m.parentNode;}function j(n,m,p,o){var r=Math.abs(n-m),q=Math.abs(p-o);return r>=q?(n-m>0?"Left":"Right"):(p-o>0?"Up":"Down");
}function l(){a=null;if(g.last){g.el.trigger("longTap");g={};}}function d(){if(a){clearTimeout(a);}a=null;}function f(){if(b){clearTimeout(b);}if(k){clearTimeout(k);
}if(h){clearTimeout(h);}if(a){clearTimeout(a);}b=k=h=a=null;g={};}i(document).ready(function(){var m,n;i(document.body).bind("touchstart",function(o){m=Date.now();
n=m-(g.last||m);g.el=i(c(o.originalEvent.touches[0].target));b&&clearTimeout(b);g.x1=o.originalEvent.touches[0].pageX;g.y1=o.originalEvent.touches[0].pageY;
if(n>0&&n<=250){g.isDoubleTap=true;}g.last=m;a=setTimeout(l,e);}).bind("touchmove",function(o){d();g.x2=o.originalEvent.touches[0].pageX;g.y2=o.originalEvent.touches[0].pageY;
}).bind("touchend",function(o){d();if((g.x2&&Math.abs(g.x1-g.x2)>30)||(g.y2&&Math.abs(g.y1-g.y2)>30)){h=setTimeout(function(){g.el.trigger("swipe");g.el.trigger("swipe"+(j(g.x1,g.x2,g.y1,g.y2)));
g={};},0);}else{if("last" in g){k=setTimeout(function(){var p=i.Event("tap");p.cancelTouch=f;g.el.trigger(p);if(g.isDoubleTap){g.el.trigger("doubleTap");
g={};}else{b=setTimeout(function(){b=null;g.el.trigger("singleTap");g={};},250);}},0);}}}).bind("touchcancel",f);i(window).bind("scroll",f);});["swipe","swipeLeft","swipeRight","swipeUp","swipeDown","doubleTap","tap","singleTap","longTap"].forEach(function(n){i.fn[n]=function(m){return this.bind(n,m);
};});})(jQuery);(function(e){document.ontouchmove=function(o){o.preventDefault();};var n={row:{},col:{}},l=_.shuffle(["abstract_macro.jpg","angry_flower.jpg","baby_rhino.jpg","bar_art.jpg","baseball.jpg","beach_scene.jpg","bike_racer.jpg","bitters.jpg","blurry.jpg","blurry_lights.jpg","boogie_boarder.jpg","bread_loafs.jpg","bread_rolls.jpg","cast_iron_food.jpg","cityscape.jpg","cityscape2.jpg","copper_coil.jpg","crazy_cat.jpg","cute_dog.jpg","derp_deer.jpg","dinner_plate.jpg","disco_ball.jpg","dj.jpg","dominican_ladies.jpg","dutch_minivan.jpg","fake_blond.jpg","foggy_coast.jpg","gorilla_in_forest.jpg","indian_lady.jpg","line_of_trees.jpg","mangy_puppy.jpg","motorcross.jpg","ninja_kick.jpg","old_lady.jpg","old_man.jpg","orchid.jpg","pig.jpg","runway_models.jpg","shadowed_horse.jpg","small_bar.jpg","smoking_model.jpg","solitary_building.jpg","solo_island.jpg","sprinters.jpg","steak_salad.jpg","street_car.jpg","sushi.jpg","transistors.jpg","ubuntu_laptop.jpg","underwater_swimmer.jpg","vegas_ny.jpg","vegas_ny_night.jpg","veggies.jpg","water_drop.jpg","wet_glass.jpg","white_people.jpg","wilting_daisies.jpg"]).slice(0,11),a=function(){l.push(l.shift());
return l[0];},h={row:6,col:6},j=function(){},m=function(){n.body.on("swipeLeft swipeRight swipeUp swipeDown",function(o){switch(o.type){case"swipeLeft":g("right");
break;case"swipeRight":g("left");break;case"swipeUp":g("down");break;case"swipeDown":g("up");break;}});},d=function(){n.tapTarget.on("click",function(o){o.stopImmediatePropagation();
g(e(this).data("slideDir"));});},b=function(){n.body.on("keydown",function(o){switch(o.which){case 37:g("left");break;case 38:g("up");break;case 39:g("right");
break;case 40:g("down");break;}});},g=function(p){function q(){n.grid.scrollTo(".row-"+h.row+" > .col-"+h.col,750,{easing:"easeOutExpo"});}function o(){n.grid.scrollTo(".row-"+h.row+" > .col-"+h.col,3000,{easing:"easeInOutBack"});
}switch(p){case"left":h.col--;break;case"right":h.col++;break;case"up":h.row--;break;case"down":h.row++;break;}if(h.row<1){h.row=11;o();}else{if(h.row>11){h.row=1;
o();}else{if(h.col<1){h.col=11;o();}else{if(h.col>11){h.col=1;o();}else{q();}}}}},c=function(){var p=parseInt(window.innerWidth),r=parseInt(window.innerHeight),q=null,o=null,s=null;
e("body, .visible-frame, .frame-col").css({width:p,height:r});e(".frame-container").css({width:(p*11)+1000,height:(r*11)+1000});q=Math.sqrt(Math.pow(p,2)+Math.pow(r,2));
o=Math.acos(((Math.pow(q,2)+Math.pow(p,2))-Math.pow(r,2))/(2*q*p))*(180/Math.PI);s=90-(2*o);n.tapTargetContainer.css({width:q,height:q,top:(q/2)*-1,"-webkit-transform":"rotate("+o+"deg) skewX(-"+s+"deg)","-moz-transform":"rotate("+o+"deg) skewX(-"+s+"deg)","-ms-transform":"rotate("+o+"deg) skewX(-"+s+"deg)","-o-transform":"rotate("+o+"deg) skewX(-"+s+"deg)",transform:"rotate("+o+"deg) skewX(-"+s+"deg)"});
n.grid.scrollTo(".row-6 > .col-6");},f=function(){var o="";_(11).times(function(q){var p=q+1;o+='<div class="frame-row row-'+p+'">';_(11).times(function(t){var s=t+1,r=a();
o+='<img class="frame-col col-'+s+'" src="assets/images/frames/'+r+'" alt="'+r.split(".")[0].replace("_"," ")+'" />';});o+="</div>";a();});e(".frame-container").html(o);
n.body=e("body");n.grid=e(".visible-frame");_(11).times(function(q){var p=q+1;n.row[p]=n.grid.find(".row-"+p);n.col[p]=n.grid.find(".col-"+p);});n.loadingOverlay=e(".loading-overlay");
n.tapTarget=e(".tap-target");n.tapTargetContainer=n.tapTarget.parent();e(window).on("orientationchange resize",function(p){c();});m();d();b();c();j();e(window).load(function(){i();
n.body.addClass("delayed");n.grid.scrollTo(".row-6 > .col-6");var p=setTimeout(function(){n.loadingOverlay.fadeOut(1000);},250);});},k=0,i=function(){var o=new Image();
o.src="assets/images/frames/"+l[k];o.onload=function(){k++;if(k<l.length){i();}};};e(function(){f();});})(jQuery);